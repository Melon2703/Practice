            ...
            <div className={cn(styles.main_information)}>
                ...
                <div className={cn(styles.userInfo)}>
                    ...
                    <div className={cn(styles.status_and_info)}>
                        <ProfileStatusWithHoocks isOwner={props.isOwner} isAuth={props.isAuth} status={props.status}
                                                 setStatus={props.setStatus}/>

                        <div>
                            {isMode && props.isOwner ?
                                <ProfileForm changeMode={() => setIsMode(!isMode)} initialValues={props.user}
                                             user={props.user} onSubmit={sendData}/> :
                                <ProfileBox isOwner={props.isOwner} isAuth={props.isAuth}
                                            changeMode={() => setIsMode(!isMode)} user={props.user}/>}
                        </div>
...
const ProfileBox = (props) => {
    ...
    //Логика для появления блока редактирования поверх сайта ( нужны рефы )
    // let showContacts = (e) => {
    ...
    return (
        <div className={cn(styles.profileBox)}>
            ...
            {props.user.lookingForAJob && (
                <span><b>Описание работы: </b> {props.user.lookingForAJobDescription} </span>)}
